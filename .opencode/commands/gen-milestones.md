...
このコマンドの引数としてドキュメントのパス（$ARGUMENTS）が与えられることを期待する。
...
1. 引数（$ARGUMENTS）で指定されたドキュメントを読む
2. docs/ 配下のドキュメントを読む
...
* 外部 API 連携なら → 実 API キーを使って本物の API で動作する状態をゴールとする（docs/infrastructure.md の「外部 API 依存の扱い」を参照）
* 認証機能なら → 開発環境で認証フローが動作する（ローカル Supabase Auth）

Stub は開発・テスト用のフォールバックであり、Milestone の goal 文に Stub 動作を書かない。goal には実 API（Claude API, OpenAI API, Slack API 等）で動作することを明示する。Stub 実装自体は docs/infrastructure.md に従い Gateway interface に対して用意するが、それは goal の達成条件ではない。
...
* 悪い例: "チェック機能が動作する（API キーなしでは Stub で動作）"（Stub 前提で書いている）
...
## 追加で変えない方がいい点
- 「手順」「設計方針」「出力フォーマット」はそのまま残す方が、Claude Code→OpenCode 移行でも diff が小さく、挙動も変わらない（frontmatter/`$ARGUMENTS`/`@`以外は一切いじらない方針でOK）。 :contentReference[oaicite:3]{index=3}

必要なら、同じ方針で `/plan` と `/pr` も「frontmatter追加＋`$ARGUMENTS`対応＋参照ファイルを `@` に置換」だけの差分で揃えられる。
::contentReference[oaicite:4]{index=4}
```

[1]: https://opencode.ai/docs/commands/ "Commands | OpenCode"
[2]: https://opencode.ai/docs/tools/ "Tools | OpenCode"
