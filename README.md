...
（このリポジトリのpackage.jsonはコピーしないため、そのファイルの中にあるpackageである"@opencode-ai/plugin": "1.2.10"をコピー先のpackage.jsonへ手動で追加してください。）
...
### A.1. バイブコーディングでタスクを自動分類する（VIBE.md）
...
[注] このリポジトリは [OpenCodeの公式](https://github.com/anomalyco/opencode) とは無関係です。


### B.2. Milestone を生成する（従来フロー）
