...
src/backend/
└── contexts/
    ├── {context-name}/          # 例: content-check, source-management, auth
    │   ├── presentation/
...
- **Context 名は kebab-case。** ドメインの言葉をそのまま使う（例: `content-check`, `source-management`, `auth`）
...
| Context A/presentation/composition → Context B/presentation/composition | **許可**（後述） |
...
#### composition 層間のクロスコンテキスト import

`presentation/composition/` は Composition Root として全層を参照してよい唯一の場所であり、他 Context への依存注入が必要な場合に限り、**他 Context の `presentation/composition/` のみ** import することを許可する。

- **許可**: `contexts/content-check/presentation/composition/` → `contexts/source-management/presentation/composition/`
- **禁止**: composition 層から他 Context の domain / application / infrastructure / loaders / actions を import すること

この例外は dependency-cruiser の `no-cross-context-import-from-composition` ルールで強制される。
...
| infrastructure | `infrastructure/adapters/` `repositories/` | Gateway / Repository 実装。外部サービス接続 |
...
- **infrastructure → domain/gateways, domain/repositories。** interface を implements する
...
|---|---|---|
| Gateway | `contexts/{context}/domain/gateways/` | `contexts/{context}/infrastructure/adapters/` |
| Repository | `contexts/{context}/domain/repositories/` | `contexts/{context}/infrastructure/repositories/` |
| 共通 Gateway | `contexts/shared/domain/gateways/` | `contexts/shared/infrastructure/adapters/` |
