...
| `pnpm verify:full` | Verifier | `pnpm verify` + Playwright E2E テスト | Yes（ローカル Supabase） |

Builder は worktree 上で動くため DB に接続できない。`pnpm verify` でセルフチェックし、E2E を含む完全検証は Verifier がメインブランチで行う。
